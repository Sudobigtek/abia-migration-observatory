...
class DynamicFieldServiceTests(TestCase):
    ...
    def test_set_and_get_field_value(self):
        ...
        data = DynamicFieldService.get_entity_dynamic_data(EntityType.MIGRANT, 1)
    ...
    def test_aggregate_rebuilds_on_save(self):
        ...
        agg = EntityDynamicData.objects.get(entity_type=EntityType.MIGRANT, entity_id=2)
        self.assertEqual(agg.data["displacement_cause"], "climate")
        self.assertIn("climate", agg.search_vector)

    def test_created_by_set_on_value(self):
        obj = DynamicFieldService.set_field_value(
            self.defn, EntityType.MIGRANT, 3, "economic", self.user
        )
