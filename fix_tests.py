...
def main():
    ...
    pg_running = 'abia-postgres' in (run("docker ps --format '{{.Names}}'", check=False) or '')
...
""")
    ...
    if pg_running and not in_docker:
        ...
        run("docker exec abia-postgres psql -U postgres -c 'CREATE DATABASE abia_app_test;' 2>/dev/null || true", check=False)
